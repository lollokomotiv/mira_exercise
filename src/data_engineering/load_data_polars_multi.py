"""
Polars-based ETL for StatsBomb national team tournaments (multi-competition).

Reads all JSONs under data/raw/statsbomb/ downloaded by fetch_statsbomb.run_nationals,
and emits combined CSVs under data/processed/statsbomb/ with the same schema used
by the existing loader (matches.csv, players.csv, teams.csv, player_appearances.csv,
player_match_stats.csv, referees.csv, venues.csv, events_meta.csv).
"""
...
OUT_DIR = Path("data/processed/statsbomb/all")
...
def build_matches_df() -> pl.DataFrame:
    ...
    for p in sorted((RAW_DIR / "matches").glob("*.json")):
        try:
            comp_id, season_id = p.stem.split("_")
        except Exception:
            comp_id = season_id = None
        matches = _load_json(p)
        for m in matches:
            rows.append(
                {
                    ...
                    "competition_id": (m.get("competition") or {}).get(
                        "competition_id"
                    ),
                    ...
                    "country_id": ((m.get("stadium") or {}).get("country") or {}).get(
                        "id"
                    ),
...
def build_teams_df(matches_df: pl.DataFrame, match_ids: List[int]) -> pl.DataFrame:
    """Collect unique team ids/names without materialising intermediate dicts."""

    # Home/away teams from matches dataframe
    ...
    # Teams appearing only inside lineups JSONs
    ...
    if lineup_rows:
        frames.append(pl.DataFrame(lineup_rows))

    # Teams referenced in events JSONs (safety net for incomplete lineups)
    ...
    if event_rows:
        frames.append(pl.DataFrame(event_rows))
...
def run() -> None:
    ...
    print("✅ Polars ETL complete (multi-competition): data/processed/statsbomb/all")
