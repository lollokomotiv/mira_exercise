"""
Create derived xref outputs for EURO24 from StatsBomb + Transfermarkt using fuzzy matching.

Inputs (expected):
- data/processed/statsbomb/euro24/players.csv
- data/processed/transfermarkt/players_euro24.csv
- data/raw/transfermarkt/clubs.csv (for club aliases)

Outputs (written to data/processed/xref/euro24/):
- club_xref_euro24.csv
- player_xref_euro24_full.csv (diagnostics)
- player_xref_euro24_review_needed.csv
- player_xref_euro24_strict.csv

This implements the matching logic previously in build_xref.py, but scoped to EURO24
and using the players_euro24 TM subset.
"""
...
XREF_DIR = Path("data/processed/xref/euro24")
...
OUT_CLUB = XREF_DIR / "club_xref_euro24.csv"
OUT_PLAYER_FULL = XREF_DIR / "player_xref_euro24_full.csv"
OUT_PLAYER_REVIEW = XREF_DIR / "player_xref_euro24_review_needed.csv"
OUT_PLAYER_STRICT = XREF_DIR / "player_xref_euro24_strict.csv"
...
def build_player_xref(
    ...
    sb = sb_players.with_columns(
        [
            pl.col("player_name").map_elements(_norm).alias("name_norm"),
    ...
    tm = tm_players.with_columns(
        [
            pl.col("name").map_elements(_norm).alias("name_norm"),
    ...
    for p in sb.to_dicts():
        ...
        for q in cands:
            ...
            # token overlap proportional bonus
...
def build_club_xref() -> pl.DataFrame:
    ...
    ).with_columns([pl.col("tm_club_name").map_elements(_norm).alias("alias")])
    return df
...
def run() -> None:
    ...
    print("✅ Wrote EURO24 xref outputs")
