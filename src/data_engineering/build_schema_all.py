"""
Builds a SQLite DB for national team tournaments (since 2018) using the same
data model as euro24.sqlite, but with window views named *_nt_window.

Inputs (same CSV structure as existing pipeline, but multi-competition):
- data/processed/statsbomb/ (matches.csv, teams.csv, players.csv, player_match_stats.csv,
  player_appearances.csv, events_meta.csv, referees.csv, venues.csv)
- data/processed/transfermarkt/transfers_nt.csv (already filtered to 75-day windows)
- data/processed/xref/player_xref.csv, player_xref_all.csv, club_xref.csv

Output DB: national_tournaments.sqlite (path configurable)
"""
...
PROC_TM = Path("data/processed/transfermarkt/all")
...
def create_schema(conn: sqlite3.Connection) -> None:
    ...
    DROP VIEW IF EXISTS v_players_transfers_nt_window_all;
    DROP VIEW IF EXISTS v_players_transfers_nt_window;
    DROP VIEW IF EXISTS transfers_nt_window;
    ...
    -- In this DB, 'transfers' already contains only rows in the desired windows
    CREATE VIEW IF NOT EXISTS transfers_nt_window AS
    SELECT * FROM transfers;

    -- Players joined with all transfers in the windows (via xref)
    CREATE VIEW IF NOT EXISTS v_players_transfers_nt_window AS
    SELECT
    ...
    LEFT JOIN transfers_nt_window t ON t.player_id = px.tm_player_id;

    -- Same join but using the wider xref (player_xref_all)
    CREATE VIEW IF NOT EXISTS v_players_transfers_nt_window_all AS
    SELECT
      sp.player_id AS statsbomb_player_id,
      sp.player_name,
      sp.team_id AS national_team_id,
...
def run(db_path: str) -> None:
    ...
    try:
        ...
        # Load transfers filtered by windows
        load_csv(conn, "transfers", PROC_TM / "transfers_all.csv")
        ...
        XREF = Path("data/processed/xref/all")
        load_csv(conn, "player_xref", XREF / "player_xref_all_strict.csv")
        load_csv(conn, "player_xref_all", XREF / "player_xref_all_full.csv")
        load_csv(conn, "club_xref", XREF / "club_xref_all.csv")
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = argparse.ArgumentParser(
        description="Build national tournaments DB (since 2018)"
    )
    p.add_argument(
        "--db",
        default="data/db/all_tournaments.sqlite",
        help="Percorso DB SQLite di output",
    )
