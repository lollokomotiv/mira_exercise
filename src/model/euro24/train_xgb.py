"""
XGBoost variants for EURO24 model (basic/complex + KFold).
"""
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Train EURO24 model with XGBoost")
    p.add_argument("--db", default="data/db/euro24.sqlite")
    p.add_argument("--out", default="artifacts/model_euro2024_basic_xgb")
    ...
    if args.variant == "basic":
        if args.cv == "kfold":
            out = args.out if args.out != "artifacts/model_euro2024_basic_xgb" else "artifacts/model_euro2024_basic_kfold_xgb"
            m = kfold_basic(args.db, out, k=args.k)
            print(f"✅ EURO24 XGB (basic KFold {m['folds']}x). MAE={m['mae_mean_eur']:,.0f}€ ±{m['mae_std_eur']:,.0f}€  RMSE={m['rmse_mean_eur']:,.0f}€ ±{m['rmse_std_eur']:,.0f}€")
            print(f"Artifacts → {out}")
        else:
            m = train_basic(args.db, args.out)
            print(f"✅ EURO24 XGB (basic) trained. RMSE={m['rmse_eur']:,.0f}€ MAE={m['mae_eur']:,.0f}€ (n_train={m['n_train']}, n_val={m['n_val']})")
            print(f"Artifacts → {args.out}")
    else:
        out_default = "artifacts/model_euro2024_complex_xgb"
        if args.cv == "kfold":
            out = args.out if args.out != "artifacts/model_euro2024_basic_xgb" else f"{out_default}_kfold"
            m = kfold_complex(args.db, out, k=args.k)
            print(f"✅ EURO24 XGB (complex KFold {m['folds']}x). MAE={m['mae_mean_eur']:,.0f}€ ±{m['mae_std_eur']:,.0f}€  RMSE={m['rmse_mean_eur']:,.0f}€ ±{m['rmse_std_eur']:,.0f}€")
            print(f"Artifacts → {out}")
        else:
            out = args.out if args.out != "artifacts/model_euro2024_basic_xgb" else out_default
            m = train_complex(args.db, out)
            print(f"✅ EURO24 XGB (complex residual) trained. RMSE={m['rmse_eur']:,.0f}€ MAE={m['mae_eur']:,.0f}€ (n_train={m['n_train']}, n_val={m['n_val']})")
            print(f"Artifacts → {out}")
    return 0
