"""
all transfer fee model (minimal, opinionated).

- Loads supervised all dataset from SQLite via model.features.build_dataset
...
- Saves artifacts under artifacts/all/

Usage
  python -m model.all.train \
...
def _feature_sets_basic(df: pd.DataFrame) -> list[dict]:
    """Build iterative feature sets for basic variant.

    Starts from minimal numeric set and adds one feature at a time.
    Steps:
      1) base: [market_value_in_eur, age, xg_per90, xa_per90]
      2) + minutes
      3) + team_id (cat)
      4) + tm_height_cm
    Only keeps columns that exist in df.
    """
...
def train_all(
    ...
    # Build dataset (all window is already enforced by build_dataset labels)
    ...
    # Ensure num_cols contains only numeric columns
    print(f"Initial numeric features: {num_cols}")
    num_cols = [c for c in num_cols if pd.api.types.is_numeric_dtype(df[c])]
    print(f"Filtered numeric features: {num_cols}")

    # Coerce numeric-like columns to numeric to avoid median errors on objects
    for col in [
        "market_value_in_eur",
        "minutes",
        "xg_per90",
    ...
    # Drop rows with missing market_value_in_eur as per preference
    if "market_value_in_eur" in df.columns:
        before = len(df)
        df = df[
            ~pd.to_numeric(df["market_value_in_eur"], errors="coerce").isna()
        ].copy()
        after = len(df)
        if before - after > 0:
            print(
                f"Dropped {before - after} rows due to missing market_value_in_eur (basic)"
            )
    ...
    val_out.to_csv(out / "val_predictions_all.csv", index=False)
...
def train_all_complex(db: str, out_dir: str) -> dict:
    ...
    val_out.to_csv(out / "val_predictions_all_complex.csv", index=False)
...
def kfold_basic(
    ...
    if "market_value_in_eur" in df.columns:
        ...
        if before - after > 0:
            print(
                f"Dropped {before - after} rows due to missing market_value_in_eur (kfold basic)"
            )
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description="Train all_tournaments transfer fee model (basic or complex)"
    )
    p.add_argument("--db", default="data/db/all_tournaments.sqlite")
    p.add_argument("--out", default="artifacts/model_all_tournaments_basic")
    ...
    if args.variant == "basic":
        if args.cv == "kfold":
            out_root = (
                args.out
                if args.out != "artifacts/model_all_tournaments_basic"
                else "artifacts/model_all_tournaments_basic_kfold"
            )
            if args.iter_features:
                # Build feature sets and iterate
                ...
                print(f"✅ KFold feature sweep completato → {out_root}")
            else:
                m = kfold_basic(args.db, out_root, alpha=args.alpha, k=args.k)
                print(
                    f"✅ All tournaments (basic KFold {m['folds']}x). MAE={m['mae_mean_eur']:,.0f}€ ±{m['mae_std_eur']:,.0f}€  RMSE={m['rmse_mean_eur']:,.0f}€ ±{m['rmse_std_eur']:,.0f}€"
                )
            ...
            if args.iter_features:
                ...
                print(f"✅ Holdout feature sweep completato → {args.out}")
            else:
                m = train_all(args.db, args.out, alpha=args.alpha)
                print(
                    f"✅ All tournaments (basic) trained. RMSE={m['rmse_eur']:,.0f}€ MAE={m['mae_eur']:,.0f}€  (n_train={m['n_train']}, n_val={m['n_val']})"
                )
                print(f"Artifacts → {args.out}")
    else:
        out_default = "artifacts/model_all_tournaments_complex"
        if args.cv == "kfold":
            out = (
                args.out
                if args.out != "artifacts/model_all_tournaments_complex"
                else f"{out_default}_kfold"
            )
            m = kfold_complex(args.db, out, k=args.k)
            print(
                f"✅ All tournaments (complex KFold {m['folds']}x). MAE={m['mae_mean_eur']:,.0f}€ ±{m['mae_std_eur']:,.0f}€  RMSE={m['rmse_mean_eur']:,.0f}€ ±{m['rmse_std_eur']:,.0f}€"
            )
            print(f"Artifacts → {out}")
        else:
            out = (
                args.out
                if args.out != "artifacts/model_all_tournaments"
                else out_default
            )
            m = train_all_complex(args.db, out)
            print(
                f"✅ All tournaments (complex residual) trained. RMSE={m['rmse_eur']:,.0f}€ MAE={m['mae_eur']:,.0f}€  (n_train={m['n_train']}, n_val={m['n_val']})"
            )
            print(f"Artifacts → {out}")
    return 0
